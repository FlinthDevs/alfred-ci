...
class Export extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    protected function orderToArray($order)
    {
        $orderData = [
            ...
            'vehicule' => [],
            'lignes' => $this->getOrderItemData($order),
     ...
     * @throws \Magento\Framework\Exception\NoSuchEntityException
     */
    protected function getPromotion($appliedRuleIds, $catalogDiscountAmount, $discountAmount, $couponCode)
    {
        $promotions = [];
        if ($appliedRuleIds) {
            foreach (explode(',', $appliedRuleIds) as $ruleId) {
                ...
                /* @var $rule \Magento\SalesRule\Model\Data\Rule */
                if ($rule) {
                    $name = $rule->getName();
