...
$node = $variables['elements']['#node'];
  ...
  if ($node->getType() == 'operation' && $variables['view_mode'] == 'full') {
    ...
    $products_information = NULL;
    if (!empty($json)) {
      if (isset($json['recherches']) && !empty($json['recherches'][0])) {
        $products_information = $searchOperationService->searchOperationFacet(
      ...
      elseif (isset($json['produits']) && !empty($json['produits'][0])) {
        $products_information = $searchOperationService->searchOperationProduct(
          $searchOperationService::SEARCH_SIZE_RESULT,
          $page_num * $searchOperationService::SEARCH_SIZE_RESULT,
    ...
    if ($products_information['total'] != 0) {
      // Generate pagination.
      $variables['product_number'] = $products_information['total'];
      $variables['products'] = [
        'total' => $products_information['total'],
        'nb_pages' => ceil($products_information['total'] / $searchOperationService::SEARCH_SIZE_RESULT),
      ];
      ...
      $products_nids = $product_services->getProductByRef($products_information['results']);

      if ($products_nids != NULL) {
        // @codingStandardsIgnoreStart
        $products_displayed = Node::loadMultiple($products_nids);
        // @codingStandardsIgnoreEnd
        $products_displayed = $product_services->sortProductsByResultPosition($products_displayed, $products_information['results']);
    ...
    $parent = NULL;
    $variables['facets'] = \Drupal::formBuilder()->getForm($form_service, $products_information, $filters, $facets);

    // Initialize pager.
    pager_default_initialize($products_information['total'], 24);
